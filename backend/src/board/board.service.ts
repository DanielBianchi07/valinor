...
export class BoardService {
  ...
  async isUserAssociatedWithBoard(boardId: number, userId: number) {
    ...
    if(count === 0) {
      throw new UnauthorizedException('User is not associated with this board');
    }
    return true;
  }
  ...
  async update(id: number, updateBoardDto: UpdateBoardDto, userId: number) {
    await this.userService.isConnectedToBoard(id, userId);
    return this.boardRepository.update(id, { name: updateBoardDto.name });
  }
