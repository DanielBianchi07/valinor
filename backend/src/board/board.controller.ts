import { Controller, Get, Post, Body, Patch, Param, Delete, UseGuards, Request } from '@nestjs/common';
import { BoardService } from './board.service';
import { CreateBoardDto } from './dto/create-board.dto';
import { UpdateBoardDto } from './dto/update-board.dto';
import * as authGuard from 'src/auth/auth/auth.guard';

@Controller('board')
export class BoardController {
  constructor(private readonly boardService: BoardService) {}

  @Post()
  @UseGuards(authGuard.AuthGuard)
  create(@Body() createBoardDto: CreateBoardDto, @Request() req: authGuard.PayloadRequest) {
    return this.boardService.create(createBoardDto, req.user.id);
  }

  @Get()
  @UseGuards(authGuard.AuthGuard)
  findAll(@Request() req: authGuard.PayloadRequest) {
    return this.boardService.findAllByUserId(req.user.id);
  }

  @Get(':id')
  @UseGuards(authGuard.AuthGuard)
  async findOne(@Param('id') id: string, @Request() req: authGuard.PayloadRequest) {
    const board = await this.boardService.findOne(+id, req.user.id);
    if(!board) return;
    board.swimlanes = board.swimlanes.sort((a, b) => a.order - b.order);
    board.swimlanes.forEach((swimlane) => {
      swimlane.cards = swimlane.cards.sort((a,b) => a.order - b.order);
    });
    return board;
  }

  @Patch(':id')
  @UseGuards(authGuard.AuthGuard)
  update(@Param('id') id: string, @Body() updateBoardDto: UpdateBoardDto, @Request() req: authGuard.PayloadRequest) {
    return this.boardService.update(+id, updateBoardDto, req.user.id);
  }

  @Delete(':id')
  @UseGuards(authGuard.AuthGuard)
  remove(@Param('id') id: string, @Request() req: authGuard.PayloadRequest) {
    return this.boardService.remove(+id, req.user.id);
  }
}
