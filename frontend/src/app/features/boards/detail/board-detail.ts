...
export class BoardDetail implements OnInit {
  ...
  private readonly matDilaog = inject(MatDialog);
  ...
  board = toSignal(
    ...
    .pipe(
      switchMap(() => this.boardService.getBoardById(
  ...
  addSwimlane() {
    if(this.swimlaneForm.invalid) return;
  ...
  onSwimlaneChanged($event: CdkDragDrop<any>): void {
    const _board = this.board();
    if(!_board) return;
    moveItemInArray(this.board()?.swimlanes || [], $event.previousIndex, $event.currentIndex)

    this.boardService
      .updateSwimlaneOrder({
        boardId: _board.id,
        item: _board.swimlanes?.map((swimlane, index) => ({
          id: swimlane.id,
          order: index,
        })) || []
  })
  ...
  onCardChange($event: CdkDragDrop<any>, swimlane: ISwimlane): void {
    ...
    _board.swimlanes?.reduce((prev: ICard[], current: ISwimlane) => {
        const cards = current.cards?.map((c, idx) => ({
          ...c,
